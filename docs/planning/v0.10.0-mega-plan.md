...
| **C6** | Hardening: cross-registry sims, Tier 2/3 additions, v20 fixture-corpus capture (XVER §9), full soak passes (all 19 Fabric + the 5-scenario Paper + 4-scenario Folia sets), benchmark run with legacy-dialect + native-v20 arms (R-4), live-matrix cell 4 (same-MC legacy rungs — R-8; crafted-frame gametests carry it) | XVER Phase 6 | everything green; **cell 3 (the guard live pull) RE-ASSIGNED to D3** (2026-08-08 deviation — it needs a live GUI client join the automation cannot drive, its sibling cells 1–2 are already D3, and no tag is pushed in this program so no publish rides on it); matrix cells 1–3 complete at D3 (R-8) |
| **D0** | ColumnTimestampCache tile redesign (`timestamp-cache-tile-redesign.md` — per-region flat tiles, u32 epoch-offset stamps, ~13x per-column heap cut with AUTO re-pointed at ~8x-disc coverage, persistence v2 + v1 migration; fresh `feat/tscache-tiles` off main AFTER C6 merges, single PR, no kill switch per its §10) | timestamp-cache-tile-redesign.md (added 2026-08-08, user directive) | its §8 gates: model-differential fuzz + clamp truth table + region/slot pins + v1→v2 migration golden, Tier 1+2, fresh-backfill/warm-rejoin/dirty-broadcast/dirty-while-offline soaks + Paper warm-rejoin, manual v1-file migration eyeball |
| **D1** | Docs: PERF Phase 5 (R5 ops note + erratum carry-through), release notes (§3.2 **+ the D0 tile-cache item from its §9**), CLAUDE.md updates (incl. the stale folia-supported "ABSENT" text in the Test Commands bullet + `PluginYmlContractTest`'s javadoc **+ D0's ColumnTimestampCache/AUTO-sizing bullets**) | PERF Phase 5 + this doc | — |
...
**D0 sequencing (added 2026-08-08, user directive):** the tile redesign branches
fresh off main only AFTER C6's integration merge (its design doc pins this — the
C6 branch carries exporter/contract changes it must not collide with), and MUST be
merged BEFORE both (a) the pre-D3 whole-program 3-Fable review round, whose scope
therefore includes D0's diff, and (b) the D3 re-ports, so the 26.1 / 1.21.11
lines carry the tile cache from their first v0.10.0 build instead of needing a
separate backport. D1's release notes carry its §9 Performance item.
