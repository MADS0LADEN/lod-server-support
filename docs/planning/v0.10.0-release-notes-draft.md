...
- **Less memory churn while serving LOD from disk (selective chunk parsing)** — When
  reading chunks from disk for LOD serving, LSS now parses only the parts of the
  chunk data it actually uses (block sections and status) and skips the rest — block
  entities, heightmaps, and other data never affect LOD output. This cuts allocation
  pressure during heavy LOD streaming. New `useSelectiveNbtParse` option (default on)
  restores full parsing if set to false. Fabric.
