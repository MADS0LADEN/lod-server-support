...
- **No Folia on this line** — Folia publishes no MC 1.21.10 build (its releases jump from 1.21.8 to 1.21.11), so the plugin declares `folia-supported: false` here. Folia players should use the 1.21.11 release.
- **No in-game Sodium options page on this line** — Sodium for MC 1.21.10 (0.7.x) predates the config API the LSS options page uses, so the page is not included. Every setting is still available in the JSON config files (`lss-client-config.json` / `lss-server-config.json`).
