...
- **Cross-version LOD serving (protocol 20)** — A client and server on different
  ...
  (`unknownBlockFallback`, default stone). Older LSS clients (v0.7.x–v0.9.x) keep
  working through the existing compatibility rungs.
- **Via-proxy mismatch guard** — When ViaVersion/ViaBackwards lets a
  different-MC-version vanilla client join, LSS now detects the mismatch and
  declines the LOD session cleanly instead of serving columns the client would
  decode as garbage. **Known hole:** when Via runs on a proxy (Velocity/Bungee)
...
- **The background store warm-up costs ~21% less CPU** — the backfill's
  parse/hash/dedup work was measured across the round at a >21% cut with
  deposit throughput unchanged.
- **Serving allocates ~30% less memory per column** (selective NBT parsing +
  buffer reuse), which lowers GC pressure on busy servers.
...
- **The cost of cross-version columns, honestly**: encoding version-independent
  identities makes serving a cold column cost ~12–15% more CPU than v0.9.x
  (measured; it was ~18% before a follow-up optimization) and the wire/store
  bytes grow ~10%. Warm store serves and backfill throughput are unaffected.
  This buys the entire cross-version feature.
- **Store rows are integrity-hashed with CRC32C** (hardware-accelerated),
  replacing a slower software hash on the warm-serve path.
