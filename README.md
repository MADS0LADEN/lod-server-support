...
| `maxConcurrentDiskReads` | `0` (auto) | Caps how many expensive region-file reads (cold, un-stored chunks) run at once, so server CPU stays bounded no matter how high you raise bandwidth — store-served LODs are never throttled by it. Auto = half the reader pool with the store on, no limit with it off. Symptom when it binds: LOD holes fill at a steady bounded rate while `/lsslod diag` shows `read_gate=K/K` with `gated=` climbing; raise the value if you have CPU headroom |
